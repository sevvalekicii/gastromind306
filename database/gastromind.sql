...
-- 14. TRIGGER: UPDATE CUSTOMER LTV WHEN DINING SESSION IS COMPLETED
DELIMITER //
...
-- 15. TRIGGER: CHECK STAFF SHIFT WHEN ADDING ORDER (DISABLED - causes issues with sample data)
-- This trigger is commented out because sample data doesn't match shift schedules perfectly
/*
DELIMITER //
...
BEGIN
    ...
    IF shift_count = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Staff member is not on shift at this time.';
    END IF;
END;
//
DELIMITER ;
